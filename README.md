...
## 📌 項目簡介
**CatchLM** 是一款專為 PTT CATCH 板（兩性溝通與吸引力板）深度訂製的 **NotebookLM 式開源智慧情感導讀與對話平台**。

本專案將兩性精華區中**自 2005 年創板以來，經萬人推爆（推文數 $\ge 99$ 的「爆」文）的 57 篇殿堂級情感神作**完整灌錄，並利用先進的 **RAG（檢索增強生成）** 技術，讓您能與歷史上的多位情感大師進行深度對話。系統能自動生成智慧導讀（Study Guide），並針對您的兩性溝通、心態提升等問題，提供極具建設性的客製化行動方案！
...
1. **🔗 尊重著作權的「引用溯源橋樑」（Fair Use Citation Bridge）**：
   - **完全合理使用（Fair Use）**：本系統不以取代原作者為目的，而是扮演「通往精華區的橋樑」。AI 的每一句回答都會強制帶上可點擊的 `[Source X]` 引用標籤。
   - **雙向網頁導流**：左側文檔列表與右側引文對照面板，均已串接 **PTT 網頁版原文超連結**。使用者點擊即可在**新分頁直接開啟原文**，有效為論壇前輩們的經典原貼文引流、活化論壇資產。
2. **🧠 技術中立與零幻覺防禦**：
   - 採用 **SQLite（目錄管理）+ ChromaDB（向量檢索）** 雙核心引擎，確保 AI 僅根據 57 篇精華文章的真實內容進行回答，100% 防禦 AI 瞎編與幻覺。
...
## 🌟 核心特色 (Features)
* **💎 純淨爆款神作集**：自動收錄 57 篇殿堂級經典情感文章（366 個語意區塊），自動過濾板規、公告等噪聲。
* **三欄式質感介面 (Tailwind CSS)**：
  * **左欄**：來源文件清單（支援一鍵點擊在新分頁開啟 PTT 原文，並可一鍵生成 Study Guide 智慧導讀大綱）。
  * **中欄**：AI 問答視窗（帶有 `[Source X]` 引用溯源徽章）。
